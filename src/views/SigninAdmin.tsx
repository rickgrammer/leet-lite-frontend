...
export default function () {
  ...
  const onSubmit = async (data: Signin) =>  {
    try {
      ...
      navigate('/questions/create', {replace: true})
    } catch (e: unknown) {
      toast.current?.show({
        severity: 'error',
  ...
  return (
    ...
    >
        ...
        <InputText id="email1" value="admin@admin.com" type="text" className="w-full mb-3"
          {...register("email")}
        />

        <label htmlFor="password1" className="block text-900 font-medium mb-2">Password</label>
        <InputText id="password1" type="password" value="admin123" className="w-full mb-3"
          {...register("password")}
        />
      ...
      <div className='flex justify-content-between'>
        ...
        <span onClick={() => navigate('/sign-in')} className='mt-3 underline cursor-pointer'>Sign in as user</span>
